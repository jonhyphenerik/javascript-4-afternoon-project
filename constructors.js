...
function CarFactory(newMake, newModel) {
  this.make=newMake;
  this.model=newModel; 
}
////////// PROBLEM 2 //////////
...
Movie.prototype.changeRating = function(newRating){this.rating = (this.rating+newRating)/2; return this.rating}

let waterworld = new Movie('WaterWorld', 'water', 55);
console.log(waterworld);
waterworld.changeRating(100);
console.log(waterworld);
  ...
  User.prototype.removeSavedPost=function(id){
    for(let item in this.savedPosts){
      if(this.savedPosts[item].id===id){
        delete this.savedPosts[item]
      }
    }
  }
